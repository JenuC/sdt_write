...
/*                              SPCM                                         */
/*---------------------------------------------------------------------------*/
/*    Copyright (c) Becker & Hickl GmbH  2000-2022 All Rights Reserved.     */
/*---------------------------------------------------------------------------*/
/*                                                                           */
/* Title   : SPCM_DEF.H                                                      */
/* Purpose : Include file for programs using SPCM  32(64)-bit DLL            */
/*              under Windows 8/10/11                                        */
/* Note    :                                                                 */
/*                                                                           */
...
/*
   ...
   SPC_FIRMWARE_VER   -42      Incorrect firmware version of DPC/SPC module
...
// up to v.5.0 (before 12.12.2022)  - X_AXIS_TYPE now = TDC_CONTROL
...
#define SYNC_FREQ TAC_LIMIT_LOW
         ...
         // new parameters defines for TDC-104 module - CFD1..4 TH or ZC
#define CFD_TH1_TDC   CFD_LIMIT_LOW
#define CFD_TH2_TDC   CFD_LIMIT_HIGH
#define CFD_TH3_TDC   CFD_ZC_LEVEL
#define CFD_TH4_TDC   SYNC_THRESHOLD
#define CFD_ZC1_TDC   TAC_LIMIT_HIGH
#define CFD_ZC2_TDC   SYNC_HOLDOFF
#define CFD_ZC3_TDC   CFD_HOLDOFF
#define CFD_ZC4_TDC   SYNC_ZC_LEVEL
...
{
  float sync_rate;   // for TDC-104 Sync/CH4 rate, for DPC-230 - total photons rate in TDC1
  float cfd_rate;    // for TDC-104 CH1 rate, for DPC-230 - total photons rate in TDC2
  float tac_rate;    // for TDC-104 CH2 rate, for DPC-230 - not used
  float adc_rate;    // for TDC-104 CH3 rate, for DPC-230 - not used
...
#define SPC_SIMUL153         153    /* simulation mode of SPC150NXX module */
#define SPC_SIMUL131         131    /* simulation mode of SPC130EM module */
#define SPC_SIMUL132         132    /* simulation mode of SPC130EMN module */
#define SPC_SIMUL135         135    /* simulation mode of SPC130IN module */
#define SPC_SIMUL136         136    /* simulation mode of SPC130INX module */
#define SPC_SIMUL137         137    /* simulation mode of SPC130INXX module */
...
#define SPC_SIMUL186         186    /* simulation mode of SPC186 module */
#define SPC_SIMUL187         187    /* simulation mode of SPC187 module */
#define DPC_SIMUL330         330    /* simulation mode of DPC330 module */
#define DPC_SIMUL590         590    /* simulation mode of DPC590 module */
#define TDC_SIMUL104         104    /* simulation mode of SPC-QC-104 module */
#define TDC_SIMUL004         004    /* simulation mode of SPC-QC-004 module */
#define TDC_SIMUL106         106    /* simulation mode of SPC-QC-106 module */
...
#define M_SPC152    152    // SPC-150NX = SPC-150N with changed TAC range ( instead of 50-5000ns):
                           ...
                           //                   SPC-150NXX (old SPC-150NX-12) 12.5-125ns
...
#define M_SPC135    135    // SPC-130IN - as SPC-130-EMN but with faster data transfer on PCI-EX bus
                           //                          (using SmartLogic IP_CORE on Xilinx Artix 7)
#define M_SPC136    136    // SPC-130INX = SPC-130IN with TAC range 25-2500ns ( instead of 50-5000ns)
#define M_SPC137    137    // SPC-130INXX = SPC-130IN with TAC range 12.5-125ns ( instead of 50-5000ns)
...
#define M_SPC182    182    // SPC-180NXX = SPC-180N with TAC range 12.5-125ns ( instead of 50-5000ns)
#define M_SPC185    185    // SPC-180N-USB - as SPC-180N with USB-3 interface
#define M_SPC186    186    // SPC-180NX-USB - as SPC-180N-USB with TAC range 25-2500ns (instead of 50-5000ns)
#define M_SPC187    187    // SPC-180NXX-USB - as SPC-180N-USB with TAC range 12.5-125ns (instead of 50-5000ns)

#define M_TDC104    104    // SPC-QC-104  - four TDC (time-to-digital)  absolute photon timing channels ( named TDC-104 further )
                           //               or three TCSPC/FLIM channels
#define M_TDC004    004    // SPC-QC-004  - four TDC (time-to-digital)  absolute photon timing channels
                           //               or three TDC TCSPC channels -  FLIM not available
#define M_TDC106    106    // SPC-QC-106  - six TDC (time-to-digital)  absolute photon timing channels
                           //                or five TDC TCSPC/FLIM channels
...
#define SPC_SEQ_GAP150   0x2000  /* SPC15x, SPC16x,18x, SPC131-7 - Sequencer is waiting for other bank to be armed
...
#define INIT_SPC_XILINX_ERR           -100  // only for SPC-930 module
      ...
      //   SPC has reduced functionality, when Xilinx chip is not configured
         ...
         /* mode values for SPC7xx, 830, 930 and 140, 15x, 16x,18x, 130-EM(N) ( 131(2)), 130IN(X/XX), TDC-104 */

#define ROUT_IN   0
#define ROUT_OUT  1  // for 830, 930, 140, 131-7 , 15x, 16x,18x, TDC-104  - FIFO mode
#define SCAN_IN   2
#define SCAN_OUT  3
#define TDC_ABS   13        // FIFO Absolute Time mode for TDC-104

#define FIFO_32M  5    // 32 bits fifo type with markers - only SPC140, 15x, 16x, 18x & 830, TDC-104
...
#define FIFO_150    7   // fifo type of SPC15x, 131-7, 16x, 18x

#define FIFO_D230   8   // fifo type of DPC230
#define FIFO_IMG    9   // fifo image type of SPC140,15x,16x,18x, 830 - like FIFO_140,
                        //      but with marker photons
#define FIFO_D590   10  // fifo type of DPC590
#define FIFO_TDC    11  // fifo type of TDC-104 module
#define FIFO_TDC_ABS    12  // fifo absolute time type of TDC-104 module
...
typedef struct _SPCdata{    /* structure for library data  */
  ...
  unsigned long tdc_control;      /*   up to v.5.0 (before 12.12.2022) it was x_axis_type
                                      bits 0-3 - reserved, in the past = x_axis_type for SPC-930 module
                                   TDC-104 module: bits 4 - 21 = control bits for TDC-104 :
                                      ...
                                      bit 21 = 1 - Suppress IN4 signal in TDC_ABS mode -  currently not used
...
typedef struct _SPC_Adjust_Para{
  short vrt1;     //   for SPC15x,16x,18x,131-7 RESET Duration
  short vrt2;     //   for SPC15x,16x,18x,131-7 SADC  Duration
  short vrt3;     //   for SPC15x,16x,18x,131-7 SADC  Delay
  short dith_g;
  float gain_1;
  float gain_2;
  float gain_4;
...
typedef struct {
  ...
  short stream_type;       // bit 0 = 1 - stream of BH .spc files ( 1st entry in each file
                           ...
                           // bit 11 = 1 - stream contains data from TDC-104 module
                           //             mt_clock unit is [fs] ( 1e-15 ) instead of ( 1e-10)
  ...
  int   mt_clock;          // macro time clock ( from the 1st photon entry ) -
                           //      unit depends on the stream_type bit 8 & 11
                           //            bit 8 & 11 = 0    unit is [0.1 ns] ( 1e-10 )
                           //            bit 8 or 11 = 1   unit is   [1 fs] ( 1e-15 ) - DPC-230 & TDC-104 data
  ...
  unsigned __int64  stream_size;  // total stream size in bytes ( all files/buffers)
                              ...
                              //   for ring buffer stream - current number of bytes not read from ring buffer
...
typedef struct {
  ...
  unsigned short rout_chan;  /* routing channel, 0-15,
                               ...
                               for TDC modules bits 8-9 = Input Channel number  */
...
#define TDC_STREAM  0x800     // stream with TDC-104 module data

#define BUF_STREAM       0x1000    // stream input from buffers ( not from files )
#define RING_BUF_STREAM  0x4000    // stream input from one ring buffer ( not from files )
...
#define STREAM_MIN_BUF_SIZE    0x2000002      // 32MB default and minimum size of a stream buffer
#define STREAM_MAX_BUF_SIZE    0x8000002      // 128MB max size of a data buffer added to the stream
...
short   CVICDECL     SPC_clear_mom_memory ( short mod_no );
short   DLLSTDCALL   SPCstd_clear_mom_memory ( short mod_no );

short   CVICDECL     SPC_prepare_time_gates ( short mod_no, short gates_no,
                         short rchan_no, int *gates, short equal_rchan );
short   DLLSTDCALL   SPCstd_prepare_time_gates ( short mod_no, short gates_no,
                         short rchan_no, int *gates, short equal_rchan );
...
short   CVICDECL     SPC_read_mom_data  ( short mod_no, short with_counters,
                                          short first_chan, short last_chan,
                                          unsigned long *data);
short   DLLSTDCALL   SPCstd_read_mom_data  ( short mod_no, short with_counters,
                                          short first_chan, short last_chan,
                                          unsigned long *data);
...
short   DLLSTDCALL   SPCstd_set_adjust_parameters (short mod_no, SPC_Adjust_Para * adjpara);
                   ...
                   // for SPC130, SPC6x0, 830, 140, 15x,16x,18x, 131-7, DPC230 modules
...
short   CVICDECL     DPC_fill_memory ( short mod_no, short page, short bank,
                                       unsigned long fill_value);
short   DLLSTDCALL   DPCstd_fill_memory ( short mod_no, short page, short bank,
                                       unsigned long fill_value);

short   CVICDECL     DPC_read_rates ( short mod_no, rate_values_dpc *rates);
short   DLLSTDCALL   DPCstd_read_rates ( short mod_no, rate_values_dpc *rates);
...
short   CVICDECL     SPC_get_fifo_init_vars ( short mod_no, short *fifo_type,
                       short *stream_type, int *mt_clock, unsigned int *spc_header);
short   DLLSTDCALL   SPCstd_get_fifo_init_vars ( short mod_no, short *fifo_type,
                       short *stream_type, int *mt_clock, unsigned int *spc_header);

short   CVICDECL     SPC_init_buf_stream ( short mod_no, short fifo_type, short stream_type,
                                           short what_to_read, int mt_clock,
                                              unsigned int   start01_offs );
short   DLLSTDCALL   SPCstd_init_buf_stream ( short mod_no, short fifo_type, short stream_type,
                                              short what_to_read, int mt_clock,
                                              unsigned int   start01_offs );

short   CVICDECL     SPC_add_data_to_stream ( short stream_hndl, void *buffer,
                                                unsigned int   bytes_no );
short   DLLSTDCALL   SPCstd_add_data_to_stream ( short stream_hndl, void *buffer,
                                                unsigned int   bytes_no );
...
short   DLLSTDCALL   SPCstd_stream_start_condition ( short stream_hndl,
                              double start_time, unsigned int   start_OR_mask,
                              unsigned int   start_AND_mask );
...
short   DLLSTDCALL   SPCstd_stream_stop_condition ( short stream_hndl,
                              double stop_time, unsigned int   stop_OR_mask,
                              unsigned int   stop_AND_mask );

short   CVICDECL     SPC_stream_reset ( short stream_hndl );
short   DLLSTDCALL   SPCstd_stream_reset ( short stream_hndl );

short   CVICDECL     SPC_get_stream_buffer_size ( short stream_hndl, unsigned short buf_no,
                                                 unsigned int *buf_size);
short   DLLSTDCALL   SPCstd_get_stream_buffer_size ( short stream_hndl, unsigned short buf_no,
                                                     unsigned int *buf_size);

short   CVICDECL     SPC_get_buffer_from_stream ( short stream_hndl, unsigned short buf_no,
                           unsigned int *buf_size, char *data_buf, short free_buf);
short   DLLSTDCALL   SPCstd_get_buffer_from_stream ( short stream_hndl, unsigned short buf_no,
                           unsigned int *buf_size, char *data_buf, short free_buf);
